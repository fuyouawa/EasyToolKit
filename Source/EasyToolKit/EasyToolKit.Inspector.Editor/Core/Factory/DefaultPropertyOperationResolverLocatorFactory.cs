...
namespace EasyToolKit.Inspector.Editor
{
    ...
    /// 基于优先级的解析器选择系统
    /// </summary>
    public class DefaultPropertyOperationResolverLocatorFactory : IPropertyOperationResolverLocatorFactory
    {
        ...
        /// <returns>匹配的操作解析器定位器</returns>
        public IPropertyOperationResolverLocator CreateLocator(InspectorProperty property)
        {
            if (property == null)
            ...
            foreach (var locatorType in sortedLocatorTypes)
            {
                try
                {
                    var locator = (IPropertyOperationResolverLocator)Activator.CreateInstance(locatorType);
                    if (locator.CanResolve(property))
                    {
                        return locator;
                    }
                }
                ...
                {
                    // 记录错误但继续尝试其他定位器
                    UnityEngine.Debug.LogWarning($"Failed to create operation locator of type {locatorType.Name}: {ex.Message}");
                }
            }

            throw new Exception($"No operation locator found for property {property.Name}");
        }

        /// <summary>
        /// 加载所有实现 IPropertyOperationResolverLocator 的类型
        /// </summary>
        private static List<Type> LoadLocatorTypes()
        {
            ...
            // 查找所有实现 IPropertyOperationResolverLocator 的具体类
            var types = currentAssembly.GetTypes()
                .Where(t => t.IsClass && !t.IsAbstract && typeof(IPropertyOperationResolverLocator).IsAssignableFrom(t))
                .ToList();
