...
namespace EasyToolKit.Core.Editor
{
    public class ReorderableList
    {
        ...
        public delegate void DrawElementDelegate(Rect rect, int index, GUIContent label, bool selected,
            bool focused);
        ...
        */
        ...
        private void DrawElements(Rect rect, Event evt)
        {
            ...
            if (!dragging)
            {
                ...
                for (int i = start; i < end; i++)
                {
                    ...
                    DrawElement(i, list.GetArrayElementAtIndex(i), GetElementDrawRect(i, elementRects[i]), selected,
            ...
            else if (evt.type == EventType.Repaint)
            {
                ...
                while (--i > -1)
                {
                    ...
                    if (element.selected)
                    {
                        DrawElement(i, element.property, element.desiredRect, true, true);
                        continue;
                    }
                    ...
                    DrawElement(i, element.property, GetElementDrawRect(i, elementRect), false, false);
        ...
        private void DrawElement(int index, SerializedProperty element, Rect rect, bool selected, bool focused)
        {
            ...
            {
                drawElementBackgroundCallback(backgroundRect, index, null, selected, focused);
            }
            else if (evt.type == EventType.Repaint)
            {
            ...
            {
                drawElementCallback(renderRect, index, label, selected, focused);
            }
            else
            {
        ...
        static class Internals
        {
            ...
            internal static Object ValidateObjectDragAndDrop(Object[] references, SerializedProperty property,
                System.Type type, bool exactType)
            {
